...
- If you're in your (insert 20's, 30's, 40's, 50's, 60's, etc) then these are the # of things you need to do to make sure you don't end up (insert pain point) by (insert age).
...
- If your getting (insert adjective) or know someone (insert adjective) there are # of incredibly important things you need to make sure you can do physically in order to (dream result).
...
- It's okay if your mom didn't talk to you about
...
- You've heard of the viral (insert method/strategy/noun name) right? Well I invented that!
...
- In this video see if you can tell what's real and what's (insert noun).
...
- Since (insert noun) from (insert place/location) are basically (insert fact), you might as well learn how to make them from home.
...
- If your (insert noun) goes (insert result) when you (insert verb) while (insert noun) and (insert adjective) really (insert result) by now you know you have (insert noun) so why aren't you doing a (insert noun) routine.
- If your (insert noun) is (insert adjective) like this, you are likely (insert diagnosis).
...
- You would need to (insert verb) (insert number) (insert noun) today to get the same (insert result) you grandparents obtained from just (insert verb) (insert number).
...
- If I told you, you would (insert result) from (insert action) you would probably believe me right? Well here's the thing…
...
- # things everyone needs to know how to fix in an (insert location).
...
- Did you know that you could (insert verb) a year's worth of (insert noun) in (insert time frame). That is crazy.
...
- # minutes ago (insert pain point) and had (insert pain point) if you want to (insert dream result), here is how you do it under # minutes.
...
- If I could suggest any tip to any (insert person) out there it would be this.
...
- We are going to test (insert noun), today on how (insert adjective) is it.
...
- I've had a (insert dream result) since I was (insert age).
...
- # things I'd do if I were to re-start (insert industry/niche/occupation/hobby).
...
- It cost (insert $) to build my own (insert noun), acting as my own general contractor. Here is the complete cost breakdown.
...
- Both these (insert noun) are exactly the same. I have not changed a single (insert noun). But this one is (insert metric) and this one is (insert metric).
...
- # things I would never (insert action) in my (insert age range) if I wanted to (insert dream result) by (insert age range).
...
- # things you should never put in your (insert noun).
...
- I only have (insert time) to finish over (insert number) of (insert noun) orders.
...
- # months/years ago my friends and I started a (insert business). And it turned out to be the biggest (insert result).
...
- # years ago my (insert person) and I started (insert action).
...
- I was walking past this (insert store) when I saw they had (insert noun).
- Fortunately my (insert person) and I have a tendency to be a little bit impulsive.
...
- I am removing all my (insert noun) friendly upgrades before we (insert action). Today we are removing (insert noun).
...
- # days ago I quit (insert noun) as a (insert age) (insert title), and started (insert noun) for literally one reason.
...
- Let me show yall how to make (insert $) per day, by installing a simple (insert noun).
...
- My customer/client got (insert dream result) without (insert pain point) and here's how.
...
- I am only (insert metric) but I have became one of the best (insert title) in the world.

# Day in the Life Hooks
...
- So okay being an (insert target audience), my days vary quite a lot from one another.
- Day in the life of a (insert adjective) person.
- Welcome back to the day in the life of two (insert label) trying to build the next (insert business).
- (insert noun) day # (insert event about that day).
- I am a # year old (insert title), and I am heading to (insert event/location).
- This is a day in the life of a (insert title) (insert noun) edition.
- Come to work with me as a (insert title).
- (insert school) day #, my last (insert noun).
- Day # of turning from (insert before state) to (insert after state) and suddenly I don't want to be (insert after state) anymore.
- Come with me to earn $ per day, with (insert avenue).
- Day # trying to make (insert $) by the end of the year, by (insert method).
- Day in the life of a future millionaire.
- Day in the life as a (insert title) (insert noun) edition.
- This is what an average day of a (insert title) looks like a week out from (insert event).
